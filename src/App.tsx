            ...
            >
              ...
              {!isAdminAuthenticated ? (
                ...
                <div className="hidden space-y-4 flex-1 flex flex-col min-h-0">
                  {/* Tab bar */}
                  <div className="flex border-b border-zinc-200 overflow-x-auto gap-1 pb-1 flex-none scrollbar-none">
                    {(['fabrics', 'rts', 'sizes', 'general'] as const).map((tab) => (
                      <button
                        key={tab}
                        onClick={() => {
                        ...
                        className={`text-[10px] font-bold py-1 px-3 rounded-full transition-colors shrink-0 ${
                          adminActiveTab === tab
                  ...
                  {/* Feedback line */}
                  {adminSuccess && (
                    <div className="text-[10px] text-emerald-700 bg-emerald-50 border border-emerald-100 p-2 rounded-xl font-bold animate-fadeIn">
                      {adminSuccess}
                    </div>
                  )}
                  {adminError && (
                    <div className="text-[10px] text-rose-700 bg-rose-50 border border-rose-100 p-2 rounded-xl font-medium">
                      {adminError}
                    </div>
                  )}

                  {/* R2 Storage Library Visibility Controller Banner */}
                  <div className="bg-[#fcfbf9] border border-zinc-200 p-3 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-3xs text-left animate-fadeIn">
                    <div className="space-y-0.5 max-w-md">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[8px] font-black text-[#7D8F7D] bg-[#7D8F7D]/10 px-1.5 py-0.5 rounded-full uppercase tracking-wider inline-block">
                          🎨 R2 Storage Photo Library
                        </span>
                      </div>
                      <h4 className="text-[10px] font-extrabold text-zinc-800 flex items-center gap-1.5 mt-1">
                        <span>👁️</span> Show/Hide R2 Photo Library in Back Office
                      </h4>
                      <p className="text-[9px] text-zinc-500 leading-relaxed font-medium">
                        Set to <strong>HIDDEN</strong> to prevent loading and fetching your live R2 photo library, saving bandwidth. Toggle to <strong>LIVE</strong> when you want to view, search, and assign photos to your fabrics.
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto shrink-0 sm:justify-end">
                      <div className="flex items-center gap-1 bg-zinc-100 p-1 rounded-xl border border-zinc-200 shadow-inner">
                        <button
                          type="button"
                          onClick={() => toggleHideLookbookInBackOffice(false)}
                          className={`px-2.5 py-1 rounded-lg text-[8px] font-black tracking-widest uppercase transition-all cursor-pointer ${
                            !hideLookbookInBackOffice
                        ...
                        <button
                          type="button"
                          onClick={() => toggleHideLookbookInBackOffice(true)}
                          className={`px-2.5 py-1 rounded-lg text-[8px] font-black tracking-widest uppercase transition-all cursor-pointer ${
                            hideLookbookInBackOffice
                  ...
                  {/* SCROLLABLE ADMIN VIEW AREA */}
                  <div className="space-y-4 overflow-y-auto max-h-[78vh] pr-1 scroll-smooth custom-scrollbar flex-1">

                                  {/* FABRICS TAB */}
                    {adminActiveTab === 'fabrics' && (
                      <div className="space-y-4 text-left">
                        
                        {/* 1. MIRROR IMAGE CATALOG LIVE GRID */}
                        <div className="bg-[#f6f3ed] border border-zinc-200/80 rounded-[28px] p-3 shadow-inner-sm">
                          <div className="flex flex-col gap-2 mb-2.5">
                            <div className="flex justify-between items-center px-1">
                              <h4 className="text-[11px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                                <span>🎨</span> Live Catalog Mirror
                              </h4>
                              <span className="text-[9px] font-extrabold text-zinc-400 bg-white/60 border border-zinc-200 px-2 py-0.5 rounded-full uppercase">
                                {adminFabricsSubTab === 'top' ? `Top Fabrics (${fabricsTop.length})` : `Backing (${fabricsBacking.length})`}
                              </span>
                            </div>
                            ...
                            <div className="flex bg-zinc-200/50 p-1 rounded-xl">
                              <button
                                type="button"
                                onClick={() => setAdminFabricsSubTab('top')}
                                className={`flex-1 py-1.5 text-[10px] font-black tracking-wider uppercase rounded-lg transition-all ${
                                  adminFabricsSubTab === 'top'
                                    ? 'bg-white text-zinc-950 shadow-3xs'
                                    : 'text-zinc-500 hover:text-zinc-800'
                                }`}
                              >
                                Top Fabrics ({fabricsTop.length})
                              </button>
                              <button
                                type="button"
                                onClick={() => setAdminFabricsSubTab('backing')}
                                className={`flex-1 py-1.5 text-[10px] font-black tracking-wider uppercase rounded-lg transition-all ${
                                  adminFabricsSubTab === 'backing'
                                    ? 'bg-white text-zinc-950 shadow-3xs'
                                    : 'text-zinc-500 hover:text-zinc-800'
                                }`}
                              >
                                Backing Fabrics ({fabricsBacking.length})
                              </button>
                            </div>
                          </div>

                          {/* SCROLLING GRID MATCHING CUSTOMER STOREFRONT */}
                          <div className="max-h-[380px] overflow-y-auto custom-scrollbar p-1">
                            {adminFabricsSubTab === 'top' ? (
                              fabricsTop.length === 0 ? (
                                <p className="text-xs text-center py-8 text-zinc-500 font-bold">No top fabrics in catalog</p>
                              ) : (
                                (() => {
                                  // Extract all unique active categories
                                  const allUniqueCats = [...categories];
                                  fabricsTop.forEach(f => {
                                    if (f.category) {
                                      const trimmed = f.category.trim();
                                      if (trimmed && !allUniqueCats.some(c => c.toLowerCase() === trimmed.toLowerCase())) {
                                  ...
                                  const getCatEmoji = (catName: string) => {
                                    ...
                                    if (lower.includes('flower') || lower.includes('floral')) return '🌸';
                                    if (lower.includes('batik')) return '🎨';
                                    if (lower.includes('kimmi')) return '👘';
                                    if (lower.includes('character') || lower.includes('unicorn')) return '🦄';
                                    if (lower.includes('geo') || lower.includes('abstract')) return '📐';
                                    if (lower.includes('halloween') || lower.includes('spooky')) return '🎃';
                                    if (lower.includes('animal') || lower.includes('cat') || lower.includes('dog')) return '🐾';
                                    if (lower.includes('solid') || lower.includes('organic')) return '🌿';
                                    if (lower.includes('new')) return '✨';
                                    if (lower.includes('leave') || lower.includes('leaving')) return '🍂';
                                    return '🏷️';
                                  };

                                  // Group fabrics top
                                  const grouped: Record<string, FabricOption[]> = {};
                                  allUniqueCats.forEach(c => {
                                    grouped[c] = [];
                                  });
                                  const uncategorized: FabricOption[] = [];

                                  fabricsTop.forEach(f => {
                                    const fCat = f.category || '';
                                    const matched = allUniqueCats.find(c => c.toLowerCase() === fCat.trim().toLowerCase());
                                    if (matched) {
                                      grouped[matched].push(f);
                                    } else {
                                      uncategorized.push(f);
                                    }
                                  });

                                  // We will display all non-empty categories, and if uncategorized is not empty, we display it too
                                  const finalCatsToRender = allUniqueCats.map(cat => ({
                                    id: cat,
                                    name: cat,
                                    emoji: getCatEmoji(cat),
                                    items: grouped[cat].sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' }))
                                  })).filter(c => c.items.length > 0);

                                  if (uncategorized.length > 0) {
                                    finalCatsToRender.push({
                                      id: 'uncategorized',
                                      name: 'Uncategorized / Others',
                                      emoji: '📦',
                                      items: uncategorized.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' }))
                                    });
                                  }

                                  return (
                                    <div className="space-y-3">
                                      {finalCatsToRender.map((group) => {
                                        const isExpanded = !!expandedAdminFabricCategories[group.id];
                                        return (
                                          <div key={group.id} className="border border-zinc-200/60 rounded-2xl overflow-hidden bg-white/75 shadow-3xs transition-all hover:border-zinc-350">
                                            {/* Category Accordion Header */}
                                            <button
                                              type="button"
                                              onClick={() => {
                                                setExpandedAdminFabricCategories(prev => ({
                                                  ...prev,
                                                  [group.id]: !prev[group.id]
                                                }));
                                              }}
                                              className="w-full flex items-center justify-between p-3 cursor-pointer hover:bg-zinc-100/60 transition-colors select-none text-left"
                                            >
                                              <div className="flex items-center gap-2">
                                                <span className="text-sm">{group.emoji}</span>
                                                <span className="text-xs font-black uppercase text-zinc-800 tracking-wider font-sans">{group.name}</span>
                                                <span className="text-[9px] font-extrabold text-brand-moss bg-brand-moss/10 px-2 py-0.5 rounded-full font-mono">
                                                  {group.items.length} items
                                                </span>
                                              </div>
                                              <ChevronDown className={`h-4 w-4 text-zinc-400 transition-transform stroke-[2.5px] ${isExpanded ? 'rotate-180' : ''}`} />
                                            </button>

                                            {/* Category Grid (Expanded) */}
                                            {isExpanded && (
                                              <div className="p-3 border-t border-zinc-150/60 bg-zinc-50/50 animate-fadeIn">
                                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                                                  {group.items.map((f) => (
                                                    <div
                                                      key={f.id}
                                                      className="bg-white rounded-[20px] border-2 transition-all relative flex flex-col justify-between select-none overflow-hidden border-zinc-200/85 shadow-3xs hover:shadow-xs"
                                                    >
                                                      {/* Full width top image */}
                                                      <div className="relative aspect-square w-full overflow-hidden bg-zinc-50 flex-none border-b border-zinc-100">
                                                        <img 
                                                          src={getOptimizedImageUrl(f.imageUrl, 'thumbnail')} 
                                                          alt={f.name} 
                                                          className="w-full h-full object-cover"
                                                          referrerPolicy="no-referrer"
                                                        />
                                                      ...
                                                      {/* Info portion */}
                                                      <div className="p-2 flex-1 flex flex-col justify-between gap-1.5 text-left">
                                                        <div className="space-y-1 w-full">
                                                          {/* Row 1: Fabric Name & Live/Hide Button */}
                                                          <div className="flex items-center justify-between gap-1">
                                                            <span className="text-[10px] font-black text-zinc-900 font-sans uppercase tracking-wide truncate flex-1" title={f.name}>
                                                              {f.name}
                                                            </span>
                                                            <div className="flex items-center gap-1 shrink-0">
                                                              <button
                                                                type="button"
                                                                onClick={async () => {
                                                                  const updated = fabricsTop.map(x => 
                                                                    x.id === f.id ? { ...x, hidden: !x.hidden } : x
                                                                  );
                                                                  setFabricsTop(updated);
                                                                  const success = await saveDatabase({
                                                                    fabricsTop: updated,
                                                                    fabricsBacking,
                                                                    sizeOptions,
                                                                    ...
                                                                    shapeOptions,
                                                                    washingFaq,
                                                                    settings: { adminPassword: activePassword, categories }
                                                                  });
                                                                  if (success) {
                                                                    setAdminSuccess(`"${f.name}" status updated to ${!f.hidden ? 'HIDDEN' : 'LIVE'}!`);
                                                                    setTimeout(() => setAdminSuccess(''), 2000);
                                                                  }
                                                                }}
                                                                className={`px-1.5 py-0.5 rounded text-[7.5px] font-black tracking-widest uppercase transition-all border cursor-pointer inline-flex items-center gap-0.5 ${
                                                                  f.hidden
                                                                    ? 'bg-rose-50 hover:bg-rose-100 text-rose-700 border-rose-200'
                                                                    : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border-emerald-200'
                                                                }`}
                                                              >
                                                                {f.hidden ? 'HIDDEN' : 'LIVE'}
                                                              </button>
                                                              <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${f.hidden ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                                                            </div>
                                                          </div>

                                                          {/* Row 2: Current category and Change Category Dropdown */}
                                                          <div className="flex flex-col gap-0.5 border-t border-zinc-100 pt-1 mt-0.5">
                                                            <div className="flex items-center justify-between gap-1">
                                                              <div className="flex flex-col min-w-0">
                                                                <span className="text-[7px] text-zinc-400 font-sans font-extrabold uppercase tracking-widest leading-none">
                                                                  Category
                                                                </span>
                                                                <span className="text-[8.5px] font-bold text-zinc-700 truncate">
                                                                  {f.category || 'None'}
                                                                </span>
                                                              </div>
                                                              <select
                                                                ...
                                                                onChange={async (e) => {
                                                                  ...
                                                                  if (success) {
                                                                    setAdminSuccess(`"${f.name}" category updated to ${val || 'None'}!`);
                                                                    setTimeout(() => setAdminSuccess(''), 2050);
                                                                  }
                                                                }}
                                                                className="p-0.5 text-[9px] border border-zinc-200 rounded-md bg-white font-bold focus:outline-hidden max-w-[65px] cursor-pointer"
                                                              >
                                                        ...
                                                        {/* Action: Save Button */}
                                                        <div className="pt-1 mt-0.5 border-t border-zinc-100/70">
                                                          <button
                                                            type="button"
                                                            onClick={async () => {
                                                              const success = await saveDatabase({
                                                            ...
                                                            className="w-full bg-brand-moss hover:bg-brand-moss/90 text-white font-black py-1 rounded text-[8.5px] uppercase tracking-wider shadow-3xs transition-all cursor-pointer text-center"
                                                          >
                                                            Save
                                                          </button>
                                                        </div>
                              ...
                              fabricsBacking.length === 0 ? (
                                <p className="text-xs text-center py-8 text-zinc-500 font-bold">No backing fabrics in catalog</p>
                              ) : (
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                                  {[...fabricsBacking]
                                    .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' }))
                                    .map((f) => {
                                      return (
                                        ...
                                        >
                                          ...
                                          <div className="p-2 flex-1 flex flex-col justify-between gap-1.5 text-left">
                                            <div className="space-y-1 w-full">
                                              {/* Row 1: Fabric Name & Live/Hide Button & Dot */}
                                              <div className="flex items-center justify-between gap-1">
                                                <span className="text-[10px] font-black text-zinc-900 font-sans uppercase tracking-wide truncate flex-1" title={f.name}>
                                                  {f.name}
                                                </span>
                                                <div className="flex items-center gap-1 shrink-0">
                                                  <button
                                                    type="button"
                                                    onClick={async () => {
                                                      const updated = fabricsBacking.map(x => 
                                                        x.id === f.id ? { ...x, hidden: !x.hidden } : x
                                                      );
                                                      setFabricsBacking(updated);
                                                      const success = await saveDatabase({
                                                        fabricsTop,
                                                        fabricsBacking: updated,
                                                        sizeOptions,
                                                        absorbencyOptions,
                                                        readyMadeStocks,
                                                        shapeOptions,
                        ...
                        {/* 2. R2 BULK SYNC IN MIDDLE */}
                        <div className="bg-stone-50 p-3.5 rounded-2xl border border-zinc-250 space-y-3 shadow-3xs text-left">
                          <div className="space-y-0.5">
                            <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                              <span>☁️</span> Cloudflare R2 Tag Bulk Sync & Import
                            </h4>
                            <p className="text-[9.5px] text-zinc-500 leading-relaxed">
                              Specify a folder/prefix tag and an optional category tag. Only assets matching BOTH tags will be retrieved, sanitized, and bulk-imported into your fabrics catalog! Prevents duplicate entries.
                            </p>
                          </div>

                          {isR2Mock && (
                            <div className="bg-amber-50 border border-amber-200 text-amber-800 text-[9.5px] p-2.5 rounded-xl flex items-start gap-1.5 leading-relaxed">
                              <span className="text-xs font-bold">⚠️</span>
                              <div>
                                <p className="font-extrabold text-[10px]">Active Offline Mock Mode</p>
                                {missingR2Vars.length > 0 ? (
                                  <>
                                    <p className="text-zinc-500 font-medium">
                                      Your live Cloudflare R2 credentials are not detected yet. To connect your actual library, click on the <strong className="text-zinc-800">Settings panel (gear icon) in the upper right corner of Google AI Studio</strong>, and add these environment variables:
                                    </p>
                                    <ul className="list-disc list-inside mt-1 space-y-0.5 font-mono text-[9px] text-amber-900 bg-amber-100/50 p-1.5 rounded-lg">
                                      <li>R2_BUCKET_NAME: {missingR2Vars.includes('R2_BUCKET_NAME') ? '❌ Missing' : '✅ Configured'}</li>
                                      <li>R2_ACCESS_KEY_ID: {missingR2Vars.includes('R2_ACCESS_KEY_ID') ? '❌ Missing' : '✅ Configured'}</li>
                                      <li>R2_SECRET_ACCESS_KEY: {missingR2Vars.includes('R2_SECRET_ACCESS_KEY') ? '❌ Missing' : '✅ Configured'}</li>
                                      <li>R2_PUBLIC_URL: {missingR2Vars.includes('R2_PUBLIC_URL') ? '❌ Missing' : '✅ Configured'}</li>
                                    </ul>
                                    <p className="text-[9px] text-zinc-450 mt-1">
                                      Once added, click the Sync button to recheck and fetch your live library!
                                    </p>
                                  </>
                                ) : (
                                  <>
                                    <p className="text-emerald-850 font-semibold bg-emerald-50/50 p-2 rounded-lg border border-emerald-100 mt-1">
                                      ✨ Cloudflare R2 Credentials: <strong className="text-emerald-700">✅ Fully Configured & Connected!</strong>
                                    </p>
                                    <p className="text-zinc-500 font-medium mt-1">
                                      However, no photos were found in your library matching the requested tags. We are displaying mock/sample fallback items instead. Try uploading photos or adjusting your search tags!
                                    </p>
                                  </>
                                )}
                              </div>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-left">
                            <div>
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Master Tag</label>
                              <input
                                type="text"
                                placeholder="e.g. wpfabrics"
                                value={bulkImportTag}
                                onChange={(e) => setBulkImportTag(e.target.value)}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none font-mono"
                              />
                            </div>
                            <div>
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Category Tag</label>
                              <input
                                type="text"
                                placeholder="e.g. floral"
                                value={bulkImportCategoryTag}
                                onChange={(e) => setBulkImportCategoryTag(e.target.value)}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none font-mono"
                              />
                            </div>
                            <div>
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Import As Category</label>
                              <select
                                value={bulkImportType}
                                onChange={(e) => {
                                  const cat = e.target.value;
                                  setBulkImportType(cat);
                                  if (cat !== 'Backing Fabric') {
                                    setBulkImportCategoryTag(getCategoryTag(cat));
                                  }
                                }}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none"
                              >
                                {categories.map((cat) => (
                                  <option key={cat} value={cat}>{cat}</option>
                                ))}
                                <option value="Backing Fabric">Backing Fabric</option>
                              </select>
                            </div>
                            <div>
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Default Material</label>
                              <select
                                value={bulkImportMaterial}
                                onChange={(e) => setBulkImportMaterial(e.target.value)}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none"
                              >
                                <option value="Cotton Woven">Cotton Woven</option>
                                <option value="Cotton Knit">Cotton Knit</option>
                                <option value="Cotton Flannel">Cotton Flannel</option>
                                <option value="Bamboo Velour">Bamboo Velour</option>
                                <option value="Athletic Wicking Jersey">Athletic Wicking Jersey</option>
                                <option value="PUL (Waterproof)">PUL (Waterproof)</option>
                          ...
                          <button
                            ...
                            className={`w-full text-white font-black py-2.5 px-4 rounded-xl text-xs uppercase tracking-widest transition-all ${
                              isBulkImporting 
                                ...
                                : 'bg-[#7D8F7D] hover:bg-[#6C7E6C]'
                            }`}
                          >
                            {isBulkImporting ? 'Syncing Library...' : '🚀 Sync & Import Tag Library'}
                          </button>

                          <div className="pt-2 border-t border-zinc-200/60">
                            <button
                              type="button"
                              onClick={handleBulkSequentialRename}
                              className="w-full bg-white hover:bg-zinc-50 text-zinc-700 font-extrabold py-2 px-3 rounded-lg text-[10.5px] uppercase tracking-wider border border-zinc-250 hover:border-zinc-350 shadow-3xs flex items-center justify-center gap-1.5 transition-all"
                            >
                              <span>🔢</span> Sequential Rename (01, 02...)
                            </button>
                          </div>
                        </div>

                        {/* 3. COLLAPSIBLE SINGLE FABRIC ADD AT THE BOTTOM */}
                        <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-3xs text-left">
                          <button
                            type="button"
                            onClick={() => setIsAddFabFormExpanded(!isAddFabFormExpanded)}
                            className="w-full p-3.5 bg-zinc-50/75 hover:bg-zinc-100 flex justify-between items-center transition-colors"
                          >
                            <h4 className="text-[10.5px] font-black uppercase text-zinc-805 tracking-wider flex items-center gap-1.5">
                              <span>➕</span> Manually Add Single Fabric Pattern
                            </h4>
                            <span className="text-[9px] font-extrabold text-zinc-400 uppercase">
                              {isAddFabFormExpanded ? '▲ Hide' : '▼ Expand Form'}
                            </span>
                          </button>

                          {isAddFabFormExpanded && (
                            <div className="p-3.5 space-y-3 border-t border-zinc-150 animate-fadeIn">
                              <div className="grid grid-cols-2 gap-2.5">
                                <div>
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Name</label>
                                  <input
                                    type="text"
                                    placeholder="e.g. Lavender Sage"
                                    value={newFabName}
                                    onChange={(e) => setNewFabName(e.target.value)}
                                    className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none"
                                  />
                                </div>
                                <div>
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Category</label>
                                  <select
                                ...
                                <div className="col-span-2">
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Material</label>
                                  <input
                                    type="text"
                                ...
                                <div className="col-span-2">
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Stock Status</label>
                                  <select
                                    value={newFabStock}
                              ...
                              <div className="space-y-1.5">
                                <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Fabric Pattern Image</label>
                                <div className="flex gap-2">
                                  <input
                                    ...
                                    className="flex-1 p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none font-mono"
                                  />
                                  <label className="bg-zinc-100 hover:bg-zinc-200 text-zinc-750 text-xs font-bold py-2 px-3 rounded-lg cursor-pointer flex items-center gap-1 border border-zinc-300 shrink-0">
                                    {isUploadingFab ? '...' : '📁 Upload'}
                                    <input
                                      type="file"
                                      accept="image/*"
                                      className="hidden"
                                      ...
                                      onChange={async (e) => {
                                        if (e.target.files && e.target.files[0]) {
                                          try {
                                            ...
                                            setAdminSuccess('Uploaded to R2 storage successfully!');
                                            setTimeout(() => setAdminSuccess(''), 2000);
                                          } catch (err: any) {
                                            setAdminError('R2 upload failed: ' + err.message);
                                          } finally {
                                ...
                                {/* Click-to-link R2 lookbook photos */}
                                <div className="pt-1 space-y-1 bg-zinc-50 border border-zinc-200 rounded-xl p-2.5">
                                  <div className="flex items-center justify-between gap-2 pb-1.5 border-b border-zinc-200">
                                    <div className="flex items-center gap-1.5 min-w-0">
                                      <span className="text-[8px] font-black text-[#7D8F7D] uppercase tracking-wider block truncate">
                                        ⚡ R2 Storage Library:
                                      </span>
                                    </div>

                                    <div className="flex items-center gap-1 bg-zinc-200/60 p-0.5 rounded-lg shrink-0">
                                      <button
                                        type="button"
                                        onClick={() => toggleHideLookbookInBackOffice(false)}
                                        className={`px-1.5 py-0.5 rounded-md text-[7px] font-black tracking-wider uppercase transition-all cursor-pointer ${
                                          !hideLookbookInBackOffice
                                            ? 'bg-emerald-600 text-white shadow-3xs'
                                  ...
                                  {!hideLookbookInBackOffice ? (
                                    isLoadingPhotos ? (
                                      <p className="text-[8px] text-zinc-400 font-medium py-2 text-center">Loading R2 photos...</p>
                                    ) : mediaPhotos.length > 0 ? (
                                      <div className="flex gap-1.5 overflow-x-auto pt-1.5 pb-1 max-w-full custom-scrollbar animate-fadeIn">
                                        {mediaPhotos.map((photo) => {
                                          const isSelected = newFabImageUrl === photo.secure_url;
                                          return (
                                            <button
                                              type="button"
                                              key={photo.public_id}
                                              onClick={() => {
                                                setNewFabImageUrl(photo.secure_url);
                                                if (!newFabName.trim() && photo.filename) {
                                                  const cleanName = photo.filename
                                                    .replace(/[_-]/g, ' ')
                                                    .replace(/\b\w/g, (char: string) => char.toUpperCase());
                                                  setNewFabName(cleanName);
                                                }
                                              }}
                                              className={`relative h-10 w-10 rounded-lg overflow-hidden border-2 flex-none transition-all ${
                                                isSelected 
                                                  ? 'border-brand-moss scale-95 shadow-xs' 
                                                  : 'border-zinc-200 hover:border-zinc-300'
                                              }`}
                                            >
                                              <img 
                                                src={getOptimizedImageUrl(photo.secure_url, 'thumbnail')} 
                                                alt={photo.filename} 
                                                className="h-full w-full object-cover"
                                                referrerPolicy="no-referrer"
                                              />
                                              {isSelected && (
                                                <div className="absolute inset-0 bg-brand-moss/30 flex items-center justify-center">
                                                  <Check className="h-3 w-3 text-white stroke-[4.5px]" />
                                                </div>
                                              )}
                                            </button>
                                          );
                                      ...
                                      <p className="text-[8px] text-zinc-400 font-medium py-2 text-center">No R2 photos found with tag "wpfabrics".</p>
                                    )
                                  ) : (
                                    <div className="text-center py-2 flex flex-col items-center justify-center gap-0.5">
                                      <p className="text-[7.5px] text-zinc-500 font-bold uppercase tracking-wider">
                                        Previews are Hidden
                                      </p>
                                    </div>
                                  )}
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  if (!newFabName.trim()) {
                                    setAdminError('Fabric Name is required.');
                                    return;
                                  }
                                  const defaultImage = 'https://images.unsplash.com/photo-1606293926075-69a00dbf9816?auto=format&fit=crop&q=80&w=200';
                                  const freshFab: FabricOption = {
                                    id: newFabName.toLowerCase().replace(/\s+/g, '-'),
                                    name: newFabName,
                                    type: newFabCategory === 'Backing Fabric' ? 'backing' : 'top',
                                    material: newFabMaterial || 'Cotton Woven',
                                    description: 'Premium hand-selected style pattern',
                                    colorHex: '#ffffff',
                                    imageUrl: newFabImageUrl || defaultImage,
                                    premium: 0,
                                    properties: [],
                                    category: newFabCategory === 'Backing Fabric' ? 'Backing Fabric' : newFabCategory,
                                    hidden: true
                                  };

                                  let updatedTop = [...fabricsTop];
                                  let updatedBacking = [...fabricsBacking];
                                  if (freshFab.type === 'top') {
                                    updatedTop.push(freshFab);
                                    setFabricsTop(updatedTop);
                                  } else {
                                    updatedBacking.push(freshFab);
                                    setFabricsBacking(updatedBacking);
                                  }

                                  const success = await saveDatabase({
                                    fabricsTop: freshFab.type === 'top' ? updatedTop : fabricsTop,
                                    fabricsBacking: freshFab.type === 'backing' ? updatedBacking : fabricsBacking,
                                    sizeOptions,
                                    absorbencyOptions,
                                    readyMadeStocks,
                                    shapeOptions,
                                  ...
                                  if (success) {
                                    ...
                                    setNewFabMaterial('');
                                    setNewFabImageUrl('');
                                    setAdminSuccess('New fabric added successfully!');
                                    setIsAddFabFormExpanded(false);
                                  }
                                }}
                                className="w-full bg-brand-moss hover:bg-brand-moss/90 text-white font-bold py-2 px-4 rounded-xl text-xs transition-colors cursor-pointer"
                              >
                                Save Pattern to Catalog
                              </button>
                            </div>
                          )}
                        </div>
                    ...
                    {/* READY MADE STOCKS (RTS) TAB */}
                    {adminActiveTab === 'rts' && (
                      <div className="space-y-4">
                        
                        {/* 1. LIVE RTS CATALOG MIRROR (SCROLLING GRID MATCHING CUSTOMER STOREFRONT) */}
                        <div className="bg-[#f6f3ed] border border-zinc-200/80 rounded-[28px] p-3 shadow-inner-sm">
                          <div className="flex flex-col gap-2 mb-2.5">
                            <div className="flex justify-between items-center px-1">
                              <h4 className="text-[11px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                                <span>📦</span> Live RTS Catalog Mirror
                              </h4>
                              <span className="text-[9px] font-extrabold text-zinc-400 bg-white/60 border border-zinc-200 px-2 py-0.5 rounded-full uppercase">
                                RTS Packs ({readyMadeStocks.length})
                              </span>
                            </div>
                          </div>

                          <div className="max-h-[380px] overflow-y-auto custom-scrollbar p-1">
                            {readyMadeStocks.length === 0 ? (
                              <p className="text-xs text-center py-8 text-zinc-500 font-bold">No RTS packs in catalog</p>
                            ) : (
                              (() => {
                                // RTS Categories list
                                const rtsTypes: ('Liner' | 'Light' | 'Moderate' | 'Heavy' | 'Extra Long')[] = ['Liner', 'Light', 'Moderate', 'Heavy', 'Extra Long'];
                                
                                // Emoji & Display label map
                                const rtsLabels: Record<string, { label: string, emoji: string }> = {
                                  'Liner': { label: 'Liners', emoji: '🌿' },
                                  'Light': { label: 'Lights', emoji: '🌸' },
                                  'Moderate': { label: 'Moderates', emoji: '🌺' },
                                  'Heavy': { label: 'Heavies', emoji: '🌹' },
                                  'Extra Long': { label: 'Extra Longs', emoji: '🌼' }
                                };
                                ...
                                // Sort group items strictly by name or size
                                const sortRts = (items: any[]) => {
                                  return [...items].sort((a, b) => {
                                    ...
                                    const sizeDiff = parseSizeInches(a.size) - parseSizeInches(b.size);
                                    if (sizeDiff !== 0) return sizeDiff;
                                    return (a.name || '').localeCompare(b.name || '', undefined, { numeric: true, sensitivity: 'base' });
                                  });
                                };

                                // Prepare final rendering categories that are not empty
                                const renderGroups = [
                                  ...rtsTypes.map(type => ({
                                    id: type,
                                    name: rtsLabels[type].label,
                                    emoji: rtsLabels[type].emoji,
                                    items: sortRts(grouped[type])
                                  })),
                                  {
                                    id: 'Others',
                                    name: 'Other RTS Packs',
                                    emoji: '📦',
                                    items: sortRts(grouped['Others'])
                                  }
                                ...
                                return (
                                  <div className="space-y-3">
                                    {renderGroups.map((group) => {
                                      const isExpanded = !!expandedAdminRtsCategories[group.id];
                                      return (
                                        <div key={group.id} className="border border-zinc-200/60 rounded-2xl overflow-hidden bg-white/75 shadow-3xs transition-all hover:border-zinc-350">
                                          {/* Category Accordion Header */}
                                          <button
                                            type="button"
                                            onClick={() => {
                                              setExpandedAdminRtsCategories(prev => ({
                                                ...prev,
                                                [group.id]: !prev[group.id]
                                              }));
                                            }}
                                          ...
                                          {/* Expanded content */}
                                          {isExpanded && (
                                            <div className="p-3 border-t border-zinc-150/60 bg-zinc-50/50 space-y-2.5 animate-fadeIn">
                                              <div className="grid grid-cols-1 gap-2.5">
                                                {group.items.map((item) => {
                                                  return (
                                                    <div 
                                                      key={item.id}
                                                      className="bg-white p-3 rounded-2xl border border-zinc-200 flex flex-col justify-between space-y-2.5 shadow-3xs relative overflow-hidden transition-all hover:border-zinc-250"
                                                    >
                                                      <div className="flex gap-2.5 text-left items-stretch">
                                                        {/* Image */}
                                                        <img 
                                                          src={getOptimizedImageUrl(item.imageUrl, 'thumbnail')} 
                                                          alt={item.name} 
                                                          className="w-24 self-stretch object-cover rounded-xl border border-zinc-150 bg-stone-50/50 p-0.5 flex-none"
                                                          referrerPolicy="no-referrer"
                                                        />
                                                        
                                                        {/* Info / Inputs */}
                                                        <div className="min-w-0 flex-1 space-y-1.5">
                                                          {/* Row 1: RTS Name & Live/Hide Button */}
                                                          <div className="flex items-center justify-between gap-1">
                                                            <span className="text-[11px] font-black text-zinc-900 font-sans uppercase tracking-wide truncate flex-1" title={item.name}>
                                                              {item.name}
                                                            </span>
                                                            <div className="flex items-center gap-1.5 shrink-0">
                                                              <button
                                                                type="button"
                                                                onClick={() => {
                                                                  const updated = readyMadeStocks.map(x => 
                                                                    x.id === item.id ? { ...x, hidden: !x.hidden } : x
                                                                  );
                                                                  setReadyMadeStocks(updated);
                                                                }}
                                                                className={`px-2 py-0.5 rounded text-[8px] font-black tracking-widest uppercase transition-all border cursor-pointer inline-flex items-center gap-0.5 ${
                                                                  item.hidden
                                                                    ? 'bg-rose-50 hover:bg-rose-100 text-rose-700 border-rose-200'
                                                                    : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border-emerald-200'
                                                                }`}
                                                              ...
                                                              <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${item.hidden ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                                                            </div>
                                                          </div>

                                                          {/* Description */}
                                                          <p className="text-[9.5px] text-zinc-500 leading-normal line-clamp-1 font-medium">
                                                            {item.description || 'Pre-crafted limited release pack ready for immediate dispatch'}
                                                          </p>

                                                          {/* Grid of Direct Dropdowns */}
                                                          <div className="grid grid-cols-2 gap-2 pt-1 border-t border-zinc-100">
                                                            {/* Product Type (Absorbency) Dropdown */}
                                                            <div className="flex flex-col gap-0.5 text-left">
                                                              <span className="text-[7.5px] text-zinc-400 font-sans font-extrabold uppercase tracking-widest leading-none">
                                                                Type
                                                                </span>
                                                              <select
                                                                value={item.absorbency || ''}
                                                                onChange={(e) => {
                                                                  const val = e.target.value;
                                                                  const calculatedName = RTS_NAME_MAP[val] || val;
                                                                  const calculatedDesc = STANDARD_DESCRIPTIONS[val] || '';
                                                                  let calculatedPrice = item.price || 15.00;
                                                                  if (val === 'Liner') calculatedPrice = 5.50;
                                                                  else if (val === 'Light') calculatedPrice = 8.00;
                                                                  else if (val === 'Moderate') calculatedPrice = 11.00;
                                                                  else if (val === 'Heavy') calculatedPrice = 14.00;
                                                                  else if (val === 'Extra long') calculatedPrice = 15.00;
                                                                ...
                                                                className="w-full p-0.5 text-[10px] border border-zinc-200 rounded-md bg-white font-bold focus:outline-hidden cursor-pointer"
                                                              >
                                                            ...
                                                            <div className="flex flex-col gap-0.5 text-left">
                                                              ...
                                                              <select
                                                                ...
                                                                onChange={(e) => {
                                                                  const val = e.target.value;
                                                                  const updated = readyMadeStocks.map(x => 
                                                                    x.id === item.id ? { ...x, size: val, sizeLabel: val } : x
                                                                  );
                                                                  setReadyMadeStocks(updated);
                                                                }}
                                                                className="w-full p-0.5 text-[10px] border border-zinc-200 rounded-md bg-white font-bold focus:outline-hidden cursor-pointer"
                                                              >
                                                                {Array.from({ length: 29 }, (_, i) => `${6 + i * 0.5} inch`).map((sz) => (
                                                            ...
                                                            <div className="flex flex-col gap-0.5 text-left">
                                                              ...
                                                              <select
                                                                value={String(Number(item.price || 15.00).toFixed(2))}
                                                                onChange={(e) => {
                                                                  const val = parseFloat(e.target.value);
                                                                  const updated = readyMadeStocks.map(x => 
                                                                    x.id === item.id ? { ...x, price: val } : x
                                                                  );
                                                                  setReadyMadeStocks(updated);
                                                                }}
                                                                className="w-full p-0.5 text-[10px] border border-zinc-200 rounded-md bg-white font-mono font-bold focus:outline-hidden cursor-pointer"
                                                              >
                                                            ...
                                                            <div className="flex flex-col gap-0.5 text-left">
                                                              ...
                                                              <select
                                                                value={String(item.quantityLeft ?? '5')}
                                                                onChange={(e) => {
                                                                  const val = parseInt(e.target.value) || 0;
                                                                  const updated = readyMadeStocks.map(x => 
                                                                    x.id === item.id ? { ...x, quantityLeft: val } : x
                                                                  );
                                                                  setReadyMadeStocks(updated);
                                                                }}
                                                                className="w-full p-0.5 text-[10px] border border-zinc-200 rounded-md bg-white font-bold focus:outline-hidden cursor-pointer"
                                                              >
                                                                {Array.from({ length: 51 }, (_, i) => String(i)).map((q) => (
                                                                  <option key={q} value={q}>{q === '0' ? '0 (OFS)' : q}</option>
                                                                ))}
                                                              </select>
                                                            </div>
                                                          </div>
                                                      ...
                                                      {/* Action Buttons */}
                                                      <div className="flex items-center justify-between pt-2 border-t border-zinc-100/70 gap-2">
                                                        <button
                                                          type="button"
                                                          onClick={async () => {
                                                            if (confirm(`Delete RTS pack "${item.name}"?`)) {
                                                          ...
                                                          className="text-[9.5px] font-black tracking-widest uppercase py-1 px-3 rounded bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 transition-colors cursor-pointer"
                                                        >
                                                          DEL
                                                        </button>
                                                        ...
                                                        <button
                                                          ...
                                                          className="flex-1 bg-brand-moss hover:bg-brand-moss/90 text-white font-black py-1 rounded text-[10px] uppercase tracking-wider shadow-3xs transition-all cursor-pointer text-center"
                                                        >
                                                          Save
                                                        </button>
                                                      </div>
                        ...
                        {/* 2. RTS R2 TAG BULK IMPORT/SYNC MODULE */}
                        <div className="bg-stone-50 p-3.5 rounded-2xl border border-zinc-250 space-y-3 shadow-3xs">
                          <div className="space-y-0.5">
                            <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5 text-left">
                              <span>☁️</span> RTS R2 Tag Bulk Sync
                            </h4>
                            <p className="text-[9.5px] text-zinc-500 leading-relaxed text-left">
                              Specify a folder/prefix and an optional category tag. Only assets matching BOTH tags will be retrieved, automatically formatted, assigned size & absorbency settings, and bulk-imported as Ready-to-Ship items!
                            </p>
                          </div>

                          {isR2Mock && (
                            <div className="bg-amber-50 border border-amber-200 text-amber-800 text-[9.5px] p-2.5 rounded-xl flex items-start gap-1.5 leading-relaxed text-left">
                              <span className="text-xs font-bold">⚠️</span>
                              <div>
                                <p className="font-extrabold text-[10px]">Active Offline Mock Mode</p>
                                {missingR2Vars.length > 0 ? (
                                  <>
                                    ...
                                    <ul className="list-disc list-inside mt-1 space-y-0.5 font-mono text-[9px] text-amber-900 bg-amber-100/50 p-1.5 rounded-lg text-left">
                                      <li>R2_BUCKET_NAME: {missingR2Vars.includes('R2_BUCKET_NAME') ? '❌ Missing' : '✅ Configured'}</li>
                                      <li>R2_ACCESS_KEY_ID: {missingR2Vars.includes('R2_ACCESS_KEY_ID') ? '❌ Missing' : '✅ Configured'}</li>
                                      <li>R2_SECRET_ACCESS_KEY: {missingR2Vars.includes('R2_SECRET_ACCESS_KEY') ? '❌ Missing' : '✅ Configured'}</li>
                                      <li>R2_PUBLIC_URL: {missingR2Vars.includes('R2_PUBLIC_URL') ? '❌ Missing' : '✅ Configured'}</li>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-left">
                            ...
                            <div>
                              ...
                              <input
                                type="text"
                                placeholder="e.g. liner"
                                value={rtsBulkImportCategoryTag}
                                onChange={(e) => setRtsBulkImportCategoryTag(e.target.value)}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none font-mono"
                              />
                            </div>
                            <div>
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Import As Category</label>
                              <select
                                value={rtsBulkImportCategory}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setRtsBulkImportCategory(val);
                                  setRtsBulkImportCategoryTag(val);
                                }}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none"
                              >
                                <option value="liner">liner</option>
                            ...
                            <div>
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Default Price (S$)</label>
                              <input
                                type="number"
                                step="0.50"
                                value={rtsBulkDefaultPrice}
                                onChange={(e) => setRtsBulkDefaultPrice(e.target.value)}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none font-mono"
                                placeholder="0.00 (uses base)"
                              />
                            </div>
                            <div className="col-span-2 text-left">
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Default Stock Qty</label>
                              <input
                                type="number"
                                value={rtsBulkDefaultQty}
                                onChange={(e) => setRtsBulkDefaultQty(e.target.value)}
                                className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-white focus:outline-none font-mono"
                              />
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            onClick={handleRtsBulkTagImport}
                            className={`w-full text-white font-black py-2.5 px-4 rounded-xl text-xs uppercase tracking-widest transition-all cursor-pointer ${
                              isRtsBulkImporting 
                                ? 'bg-zinc-400 cursor-not-allowed animate-pulse' 
                                : 'bg-[#7D8F7D] hover:bg-[#6C7E6C]'
                            }`}
                          >
                            {isRtsBulkImporting ? 'Syncing RTS...' : '🚀 Sync & Import RTS Tag'}
                          </button>
                        </div>

                        {/* 3. COLLAPSIBLE SINGLE RTS ADD AT THE BOTTOM */}
                        <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-3xs text-left">
                          <button
                            type="button"
                            onClick={() => setIsAddRtsFormExpanded(!isAddRtsFormExpanded)}
                            className="w-full py-3 px-4 flex justify-between items-center hover:bg-zinc-50 transition-colors cursor-pointer text-left font-black text-[10px] uppercase text-zinc-800 tracking-wider"
                          >
                            <span className="flex items-center gap-1.5">
                              <span>➕</span> Manually Add Single RTS Pack
                            </span>
                            <span className="text-[12px] font-black text-zinc-400">
                              {isAddRtsFormExpanded ? '−' : '+'}
                            </span>
                          </button>
                          ...
                          {isAddRtsFormExpanded && (
                            <div className="p-3.5 space-y-3 border-t border-zinc-150 animate-fadeIn">
                              <div className="grid grid-cols-2 gap-2.5">
                                <div className="text-left">
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Product Type *</label>
                                  <select
                                    value={newRtsAbsorbency}
                                    onChange={(e) => {
                                      const type = e.target.value;
                                      setNewRtsAbsorbency(type);
                                      // Auto-set standard description based on type
                                      const standardDesc = STANDARD_DESCRIPTIONS[type] || '';
                                      setNewRtsDescription(standardDesc);

                                      // Auto-set standard base prices based on type
                                      if (type === 'Liner') setNewRtsPrice('5.50');
                                      else if (type === 'Light') setNewRtsPrice('8.00');
                                      else if (type === 'Moderate') setNewRtsPrice('11.00');
                                      else if (type === 'Heavy') setNewRtsPrice('14.00');
                                      else if (type === 'Extra long') setNewRtsPrice('15.00');
                                    }}
                                    className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none cursor-pointer font-semibold text-zinc-800"
                                  >
                                ...
                                <div className="text-left">
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Price (S$) *</label>
                                  <select
                                    value={newRtsPrice}
                                    onChange={(e) => setNewRtsPrice(e.target.value)}
                                    className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none font-mono text-zinc-800"
                                  >
                                    {newRtsAbsorbency === 'Liner' && <option value="5.50">5.50</option>}
                                    {newRtsAbsorbency === 'Light' && <option value="8.00">8.00</option>}
                                    {newRtsAbsorbency === 'Moderate' && <option value="11.00">11.00</option>}
                                    {newRtsAbsorbency === 'Heavy' && <option value="14.00">14.00</option>}
                                    {newRtsAbsorbency === 'Extra long' && (
                                      <>
                                        <option value="15.00">15.00</option>
                                        <option value="16.00">16.00</option>
                                        <option value="17.00">17.00</option>
                                    ...
                                    {!['Liner', 'Light', 'Moderate', 'Heavy', 'Extra long'].includes(newRtsAbsorbency) && (
                                      <>
                                        <option value="5.50">5.50</option>
                                        <option value="8.00">8.00</option>
                                        <option value="11.00">11.00</option>
                              ...
                              <div className="text-left">
                                <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Size Quick Select</label>
                                <select
                                  value={Array.from({ length: 29 }, (_, i) => `${6 + i * 0.5} inch`).includes(newRtsSize) ? newRtsSize : ""}
                                  onChange={(e) => {
                                    if (e.target.value) {
                                      setNewRtsSize(e.target.value);
                                    }
                                  }}
                                  className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none cursor-pointer text-zinc-800"
                                >
                                  <option value="">-- Quick select size --</option>
                                  {Array.from({ length: 29 }, (_, i) => `${6 + i * 0.5} inch`).map((sz) => (
                                    <option key={sz} value={sz}>{sz}</option>
                                  ))}
                                </select>
                              ...
                              <div className="text-left">
                                <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Short Description</label>
                                <select
                                  value={newRtsDescription}
                                  onChange={(e) => setNewRtsDescription(e.target.value)}
                                  className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none text-zinc-800"
                                >
                                  <option value="">-- Select standard description --</option>
                                  <option value="For: Daily freshness, spotting, very light days, or as backup for cups and tampons. (A simple place to begin if you’re new to reusable pads.)">
                                    🌿 Liner - Daily freshness...
                                  </option>
                                  <option value="For: Light flow days, the beginning or end of your period, spotting, or everyday backup. (Comfortable, slim, and easy to wash. Perfect for lighter days.)">
                                    🌸 Light - Light flow days...
                                  </option>
                                  <option value="For: Regular flow days when you need dependable everyday protection. (A balanced choice for most period days, combining comfort with reliable absorbency.)">
                                    🌺 Moderate - Regular flow...
                                  </option>
                                  <option value="For: Heavy flow days, longer wear, or extra confidence when you need it most. (Designed with added absorbency while remaining soft, breathable, and comfortable.)">
                                    🌹 Heavy - Heavy flow days...
                                  </option>
                                  <option value="For: Overnight use, postpartum recovery, or anyone who prefers extra length and coverage. (Maximum coverage and confidence for your heaviest moments.)">
                                    🌼 Extra Long - Overnight use...
                                  </option>
                                  {newRtsDescription && !Object.values(STANDARD_DESCRIPTIONS).includes(newRtsDescription) && (
                                    <option value={newRtsDescription}>{newRtsDescription}</option>
                                  )}
                                </select>
                              </div>

                              <div className="text-left">
                                <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">
                                  Notes (Admin Only) <span className="text-zinc-400 font-normal lowercase italic">(not visible to customers)</span>
                                </label>
                                <textarea
                                  placeholder="e.g. Fabric from SG designer, stored in cabinet shelf 3"
                                  value={newRtsNotes}
                                  onChange={(e) => setNewRtsNotes(e.target.value)}
                                  rows={2}
                                  className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none resize-none"
                                />
                              </div>

                              <div className="space-y-1.5 text-left">
                                <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Photo *</label>
                                {newRtsImageUrl && (
                                  <div className="flex items-center gap-2.5 p-2 bg-zinc-50 rounded-xl border border-zinc-200">
                                    <img 
                                      src={getOptimizedImageUrl(newRtsImageUrl, 'thumbnail')} 
                                      alt="Preview" 
                                      className="h-16 w-16 object-cover rounded-lg border border-zinc-300 shadow-3xs"
                                      referrerPolicy="no-referrer"
                                    />
                                    <div className="text-left text-[10px]">
                                      <span className="font-bold text-zinc-800 block">Current Photo Preview</span>
                                      <span className="text-zinc-500 font-mono block break-all text-[8px] max-w-[200px] leading-tight mt-0.5">{newRtsImageUrl}</span>
                                    </div>
                                  </div>
                                )}
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  const freshRts = {
                                    id: 'rts-' + Date.now(),
                                    name: calculatedName,
                                    description: newRtsDescription || 'Pre-crafted limited release pack ready for immediate dispatch',
                                    price: parseFloat(newRtsPrice) || 15.00,
                                    quantityLeft: 99,
                                    size: newRtsSize || '8 inch',
                                    sizeLabel: newRtsSize || '8 inch',
                                    ...
                                    absorbency: newRtsAbsorbency,
                                    absorbencyLabel: newRtsAbsorbency,
                                    imageUrl: newRtsImageUrl || defaultRtsImage,
                                    adminNotes: newRtsNotes,
                                    hidden: true
                                  };
                                  const updated = [...readyMadeStocks, freshRts];
                                  ...
                                  if (success) {
                                    ...
                                    setNewRtsPrice('15.00');
                                    setNewRtsSize('Regular Day (8")');
                                    setNewRtsPrint('Sunglow Floral');
                                    setNewRtsAbsorbency('Moderate dry');
                                    setNewRtsQuantity('5');
                                    setNewRtsNotes('');
                                    setIsAddRtsFormExpanded(false);
                                    setAdminSuccess('RTS item added successfully!');
                                    setTimeout(() => setAdminSuccess(''), 2000);
                                  }
                                }}
                                className="w-full bg-brand-moss hover:bg-brand-moss/90 text-white font-bold py-2 px-4 rounded-xl text-xs transition-colors cursor-pointer"
                              >
                                Save RTS Item to Storefront
                              </button>
                            </div>
                          )}
                        </div>
                    ...
                    {/* SIZES TAB */}
                    {adminActiveTab === 'sizes' && (
                      <div className="space-y-4">
                        <div className="bg-emerald-50 text-emerald-800 p-2.5 rounded-xl border border-emerald-150 text-[10px] leading-relaxed text-left">
                          ⚙️ Modify default pricing or core lengths for customized lines. Changes update customizer pricing immediately!
                        </div>
                        <div className="space-y-3">
                          {sizeOptions.map((sz, index) => (
                            <div key={sz.id} className="bg-white p-3 rounded-2xl border border-zinc-200/90 space-y-2 text-left">
                              <div className="flex justify-between items-center">
                                <p className="font-bold text-xs text-zinc-850 uppercase tracking-wide">{sz.name}</p>
                                <p className="text-[9px] text-zinc-400 font-mono">ID: {sz.id}</p>
                              </div>
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Base Price (S$)</label>
                                  <input
                                    type="number"
                                    step="0.50"
                                    value={sz.priceBase}
                                    onChange={(e) => {
                                      ...
                                      const updated = sizeOptions.map((s, i) => i === index ? { ...s, priceBase: updatedVal } : s);
                                      setSizeOptions(updated);
                                    }}
                                    className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 font-mono focus:outline-none"
                                  />
                                </div>
                                <div>
                                  <label className="text-[9px] text-zinc-500 font-extrabold uppercase">Default Length (")</label>
                                  <input
                                    type="number"
                                    value={sz.lengthInches}
                                    onChange={(e) => {
                                      const updatedVal = parseInt(e.target.value) || 0;
                                      const updated = sizeOptions.map((s, i) => i === index ? { ...s, lengthInches: updatedVal } : s);
                                      setSizeOptions(updated);
                                    }}
                                    className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 font-mono focus:outline-none"
                                  />
                              ...
                              <div>
                                <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Description</label>
                                <input
                                  type="text"
                                  value={sz.description}
                                  onChange={(e) => {
                                    const updatedVal = e.target.value;
                                    const updated = sizeOptions.map((s, i) => i === index ? { ...s, description: updatedVal } : s);
                                    setSizeOptions(updated);
                                  }}
                                  className="w-full p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none"
                                />
                        ...
                        <button
                          ...
                          className="w-full bg-brand-moss hover:bg-brand-moss/90 text-white font-bold py-2 px-4 rounded-xl text-xs transition-colors shadow-3xs"
                        >
                          Save Changes to Pricing
                        </button>
                      </div>
                    )}
                    ...
                    {adminActiveTab === 'general' && (
                      <div className="space-y-4">
                        {/* Order Receipt Routing Form */}
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-3 shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                            <span>📧</span> Order Receipt Routing Settings
                          </h4>
                          <p className="text-[9.5px] text-zinc-500 leading-normal">
                            Configure where custom order requests are routed when a customer clicks <strong>Send via WhatsApp</strong> or <strong>Send via Email</strong> at checkout.
                          </p>
                          <div className="space-y-3">
                            <div className="space-y-1">
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Merchant Email Address (For Orders)</label>
                              <input
                                type="email"
                                placeholder="nilam1978@gmail.com"
                                value={merchantEmail}
                                onChange={(e) => setMerchantEmail(e.target.value)}
                                className="w-full p-2.5 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none"
                              />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Merchant WhatsApp Phone (With Country Code)</label>
                              <input
                                type="text"
                                placeholder="e.g. 6583397556 or 6012345678"
                                value={merchantPhone}
                                onChange={(e) => setMerchantPhone(e.target.value)}
                                className="w-full p-2.5 text-xs border border-zinc-250 rounded-lg bg-zinc-50 font-mono focus:outline-none"
                              />
                            </div>
                            <button
                              type="button"
                              onClick={async () => {
                                if (!merchantEmail.trim()) {
                                  setAdminError('Merchant email address cannot be empty.');
                                  return;
                                }
                                const success = await saveDatabase({
                                  fabricsTop,
                                ...
                                if (success) {
                                  setAdminSuccess('Merchant contact info updated successfully!');
                                }
                              }}
                              className="w-full bg-[#7D8F7D] hover:bg-[#6C7E6C] text-white font-bold py-2 px-3 rounded-lg text-xs transition-colors shadow-3xs"
                            >
                        ...
                        {/* Security Password Lock Form */}
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-3 shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider">🔒 Security Settings</h4>
                          <div className="space-y-1.5">
                            <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">Change Secret Admin Password</label>
                            <input
                              type="password"
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                setActivePassword(newAdminPassword);
                                localStorage.setItem('admin_session_auth', newAdminPassword);
                                const success = await saveDatabase({
                                  fabricsTop,
                                  fabricsBacking,
                                  sizeOptions,
                                ...
                                if (success) {
                                  setNewAdminPassword('');
                                  setAdminSuccess('Admin password updated successfully!');
                                }
                              }}
                              className="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-2 px-3 rounded-lg text-xs transition-colors"
                            >
                              Save New Password
                            </button>
                          </div>
                        ...
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-3 shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                            <span>🏷️</span> Category List Settings
                          </h4>
                          <div className="space-y-1.5">
                            <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">
                              Active Categories (One Per Line)
                            </label>
                            <textarea
                              rows={5}
                              ...
                              placeholder="Flowers&#10;Kimmi&#10;Characters"
                              className="w-full p-2.5 text-xs border border-zinc-250 rounded-lg bg-zinc-50 font-sans focus:outline-none leading-relaxed"
                            />
                            <p className="text-[9px] text-zinc-400 leading-normal">
                              Edit, add, or remove categories. Note: Changes will instantly update the customizer filter bar and admin entry dropdown menus.
                            </p>
                            <button
                              type="button"
                              onClick={async () => {
                                ...
                                if (success) {
                                  setAdminSuccess('Categories configuration saved successfully!');
                                }
                              }}
                              className="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-2 px-3 rounded-lg text-xs transition-colors"
                            >
                        ...
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-3 shadow-3xs text-left">
                          ...
                          <div className="space-y-2.5">
                            ...
                            <div className="flex items-center gap-3 bg-zinc-50 p-3 rounded-xl border border-zinc-150">
                              <div className="h-16 w-16 bg-white border border-zinc-200 rounded-full flex items-center justify-center overflow-hidden p-0 shadow-sm shrink-0">
                                <ShopLogo url={shopLogoUrl} />
                              </div>
                              <div className="space-y-1 text-left min-w-0 flex-1">
                                <p className="text-[11px] font-bold text-zinc-850 truncate">
                                  {shopLogoUrl ? 'Custom Logo (R2 Storage)' : 'Default Logo'}
                                </p>
                                <p className="text-[9px] text-zinc-450 truncate">
                                  {shopLogoUrl || 'Fallback code is active'}
                                </p>
                                {shopLogoUrl && (
                                  <button
                                    type="button"
                                    onClick={async () => {
                                      ...
                                      if (success) {
                                        setAdminSuccess('Shop Logo reset to default!');
                                        setTimeout(() => setAdminSuccess(''), 2000);
                                      }
                                    }}
                                    className="text-[9.5px] font-bold text-red-600 hover:text-red-700 hover:underline"
                                  >
                                    Reset to Default
                                  </button>
                                )}
                            ...
                            <div className="space-y-1.5">
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase block">
                                Upload New Logo Image from Phone
                              </label>
                              <div className="flex gap-2">
                                <input
                                  type="text"
                                  placeholder="Direct Image URL (R2 Storage, etc.)..."
                                  value={shopLogoUrl}
                                  onChange={(e) => setShopLogoUrl(e.target.value)}
                                  className="flex-1 p-2 text-xs border border-zinc-250 rounded-lg bg-zinc-50 focus:outline-none font-mono"
                                />
                                <label className="bg-[#7D8F7D] hover:bg-[#6C7E6C] text-white text-xs font-bold py-2 px-3 rounded-lg cursor-pointer flex items-center gap-1 shrink-0 shadow-3xs transition-colors">
                                    ...
                                    onChange={async (e) => {
                                      if (e.target.files && e.target.files[0]) {
                                        try {
                                          ...
                                          // Save immediately to backend on upload completion!
                                          const success = await saveDatabase({
                                            fabricsTop,
                                            fabricsBacking,
                                            sizeOptions,
                                          ...
                                          if (success) {
                                            setAdminSuccess('Shop Logo uploaded and saved to R2 successfully!');
                                            setTimeout(() => setAdminSuccess(''), 3000);
                                          }
                                        } catch (err: any) {
                                          setAdminError('R2 upload failed: ' + err.message);
                              ...
                              <p className="text-[9px] text-zinc-400 leading-normal">
                                Tap <strong>Upload Logo</strong> to select your original logo file from your phone's photo library/files. It will be securely stored and update across the entire site.
                              </p>
                            </div>
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                if (success) {
                                  setAdminSuccess('Shop Logo URL updated successfully!');
                                }
                              }}
                              className="w-full bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-2 px-3 rounded-lg text-xs transition-colors"
                            >
                        ...
                        {/* Cloudflare R2 Storage Info Status Card */}
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-2.5 text-xs shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                            <span className={isR2Mock ? "text-amber-500 animate-pulse" : "text-emerald-500"}>●</span> {isR2Mock ? "Offline Mock Service" : "Integrations Server"}
                          </h4>
                          <div className="space-y-1.5 text-zinc-650 text-[10.5px] leading-relaxed">
                            <p><strong>Cloudflare R2 Storage Media Engine:</strong> {isR2Mock ? (
                              <span className="text-amber-600 font-bold bg-amber-50 border border-amber-100 px-1.5 py-0.5 rounded">⚠️ OFFLINE MOCK MODE (.env variables missing)</span>
                            ) : (
                              <span className="text-emerald-700 font-bold bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded">🟢 Connected & Live</span>
                            )}</p>
                            {isR2Mock ? (
                              <p className="text-[10px] text-zinc-500 leading-normal">
                                Your real Cloudflare R2 credentials are not active, so the app is serving sample mock assets. Re-create or restore your <code className="bg-zinc-100 px-1 rounded">.env</code> configuration with correct API keys to load live photos.
                              </p>
                            ) : (
                              <p className="text-[10px] text-zinc-500 leading-normal">
                                Upload files directly from your phone's camera roll or desktop files, they automatically upload to Cloudflare R2 and populate the secure asset URLs.
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-2.5 text-xs shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                            <span className={firebaseStatus?.connected ? "text-emerald-500" : "text-amber-500 animate-pulse"}>●</span> Firebase Firestore
                          </h4>
                          <div className="space-y-1.5 text-zinc-650 text-[10.5px] leading-relaxed">
                            <p>
                              <strong>Database Sync Mode:</strong>{' '}
                              {firebaseStatus?.connected ? (
                                <span className="text-emerald-700 font-bold bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded">🟢 Connected & Active</span>
                              ) : (
                                <span className="text-amber-600 font-bold bg-amber-50 border border-amber-100 px-1.5 py-0.5 rounded">🟡 Local Fallback Mode Active</span>
                              )}
                            </p>
                            
                            {firebaseStatus && !firebaseStatus.connected && (
                              <div className="bg-amber-50/50 border border-amber-100 p-2.5 rounded-xl space-y-1.5 mt-1 text-[10px] text-zinc-600 leading-normal">
                                <p className="font-extrabold text-amber-900 text-[10.5px]">
                                  {firebaseStatus.error?.includes('5 NOT_FOUND') 
                                    ? '📦 Firestore Database Not Created Yet' 
                                    : '🔑 Environment Credential Status'}
                                </p>
                                <p className="text-zinc-500 font-medium leading-relaxed">
                                  {firebaseStatus.explanation || 'We detected your service credentials, but are unable to perform read/write queries on Firestore yet.'}
                                </p>
                                <div className="p-1.5 bg-amber-100/45 rounded border border-amber-200/50 space-y-0.5 font-mono text-[9px] text-amber-900 leading-relaxed">
                                  <div>Project ID: <span className="font-bold">{firebaseStatus.projectId || 'wonderpads-store'}</span></div>
                                  <div>Credentials Loaded: <span className="font-bold">{firebaseStatus.envDetected?.FIREBASE_SERVICE_ACCOUNT_KEY ? '✅ Verified (2380+ bytes)' : '❌ Not Configured'}</span></div>
                                </div>
                                <p className="text-[9px] text-zinc-400 font-medium">
                                  No worries! Wonder Pads will run flawlessly using your high-speed local fallback database <code className="bg-zinc-100 px-1 rounded">customizer-db.json</code>, so you can customize and manage your fabric lines with complete peace of mind.
                                </p>
                              </div>
                            )}
                            ...
                            {firebaseStatus?.connected && (
                              <p className="text-[10px] text-zinc-500 leading-normal">
                                Catalog configurations (fabrics, sizes, categories, inventory status) are safely and instantly synced to project <code className="bg-zinc-100 px-1 rounded">{firebaseStatus.projectId}</code> across all active sessions.
                              </p>
                            )}
                                  ...
                                  fetch('/api/firebase-status')
                                    ...
                                    .then((data) => {
                                      ...
                                      if (data.connected) {
                                        ...
                                        alert('ℹ️ Firebase status updated! Showing current database fallback.');
                                      }
                                    })
                                    .catch((err) => {
                                      console.error('Failed to reload firebase status:', err);
                                      alert('Failed to contact server status API.');
                                    });
                                }}
                                className="bg-zinc-100 hover:bg-zinc-200 text-zinc-800 font-extrabold py-1 px-2 rounded-md text-[9px] uppercase tracking-wider transition-colors cursor-pointer inline-flex items-center gap-1 border border-zinc-200"
                              >
                        ...
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-3 shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-zinc-800 tracking-wider flex items-center gap-1.5">
                            <span>💾</span> Database Backup & Sync
                          </h4>
                          <p className="text-[10px] text-zinc-500 leading-normal">
                            Since Cloud Run / Cloudflare containers are temporary and reset when restarted, you can backup your catalog (fabrics, sizes, categories, stock) as a file or restore it anytime.
                          </p>
                          <div className="flex flex-col gap-2.5">
                            <button
                              ...
                              onClick={() => {
                                ...
                                setAdminSuccess('Database exported successfully as customizer-db.json!');
                              }}
                            ...
                            >
                              <span>📥</span> Export & Download customizer-db.json
                            </button>

                            <div className="border-t border-dashed border-zinc-200 pt-2.5">
                              <label className="text-[9px] text-zinc-500 font-extrabold uppercase block mb-1">
                              ...
                              <label className="w-full border border-zinc-250 hover:bg-zinc-50 text-zinc-700 text-xs font-bold py-2 px-3 rounded-lg cursor-pointer flex items-center justify-center gap-1.5 transition-colors">
                                <span>📤</span> Select & Upload customizer-db.json
                                <input
                                  type="file"
                                  accept=".json"
                                  className="hidden"
                                      ...
                                      reader.onload = async (event) => {
                                        try {
                                          ...
                                          if (parsed.fabricsTop || parsed.readyMadeStocks || parsed.sizeOptions) {
                                            ...
                                            if (success) {
                                              setAdminSuccess('Database fully restored from customizer-db.json backup!');
                                            }
                                          } else {
                                            setAdminError('Invalid customizer-db.json structure.');
                                          }
                                        } catch (err: any) {
                                          setAdminError('Failed to parse backup file: ' + err.message);
                                        }
                                      };
                        ...
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-3 shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-[#24292f] tracking-wider flex items-center gap-1.5">
                            ...
                            <span>GitHub Sync & Publish</span>
                          </h4>
                          <p className="text-[10px] text-zinc-500 leading-normal">
                            Keep your GitHub repository completely in sync! Commit and push your live fabric database updates directly back to your GitHub repository with one click.
                          </p>

                          <div className="space-y-2.5">
                            {/* Toggle Config Fields */}
                            <details className="group border border-zinc-200 rounded-xl overflow-hidden bg-zinc-50/50">
                              <summary className="list-none flex items-center justify-between p-2.5 cursor-pointer text-[10.5px] font-bold text-zinc-700 hover:bg-zinc-100 select-none transition-colors">
                                <span className="flex items-center gap-1.5">⚙️ Repository Configuration</span>
                            ...
                            <button
                              ...
                              className="w-full bg-zinc-900 hover:bg-zinc-850 disabled:bg-zinc-400 text-white font-bold py-2 px-3 rounded-lg text-xs transition-colors shadow-3xs flex items-center justify-center gap-1.5"
                            >
                              {isPublishingToGithub ? (
                                ...
                                <>
                                  ...
                                  <span>Push Database to Live GitHub Repository</span>
                                </>
                              )}
                            </button>
                          </div>
                        ...
                        {/* Emergency Restore Defaults Button */}
                        <div className="bg-white p-3.5 rounded-2xl border border-zinc-200 space-y-2.5 shadow-3xs text-left">
                          <h4 className="text-[10.5px] font-black uppercase text-red-700 tracking-wider">⚠️ System Maintenance</h4>
                          <p className="text-[10px] text-zinc-500 leading-relaxed">
                            Need to start over? This deletes all custom changes and restores Wonder Pads to the default fabric styles, pricing, and FAQs.
                          </p>
                          {isConfirmingResetData ? (
                            <div className="bg-red-50/70 border border-red-200 p-3 rounded-xl space-y-2.5 text-xs animate-fadeIn">
                              <p className="font-extrabold text-red-800 text-[10px] leading-relaxed">
                                Are you absolutely sure? This will delete all custom changes and restore default fabric catalog styles, pricing tiers, and FAQs. This action cannot be undone.
                              </p>
                              <div className="flex gap-2">
                                <button
                                  type="button"
                                  onClick={async () => {
                                    setFabricsTop(FABRICS_TOP);
                                    setFabricsBacking(FABRICS_BACKING);
                                    setSizeOptions(SIZE_OPTIONS);
                                    setAbsorbencyOptions(ABSORBENCY_OPTIONS);
                                    setReadyMadeStocks(READY_MADE_STOCKS);
                                    setShapeOptions(SHAPE_OPTIONS);
                                    setWashingFaq(WASHING_FAQ);
                                    setActivePassword('wonderpads2026');

                                    const success = await saveDatabase({
                                      fabricsTop: FABRICS_TOP,
                                      fabricsBacking: FABRICS_BACKING,
                                      sizeOptions: SIZE_OPTIONS,
                                      absorbencyOptions: ABSORBENCY_OPTIONS,
                                      readyMadeStocks: READY_MADE_STOCKS,
                                      shapeOptions: SHAPE_OPTIONS,
                                      washingFaq: WASHING_FAQ,
                                      settings: { adminPassword: 'wonderpads2026' }
                                    });

                                    setIsConfirmingResetData(false);
                                    if (success) {
                                      setAdminSuccess('Database fully reset to factory defaults.');
                                    }
                                  }}
                                  className="bg-red-600 hover:bg-red-700 text-white font-extrabold px-3 py-1.5 rounded-lg text-[10.5px] cursor-pointer"
                                >
                                  Yes, reset all
                                </button>
                                <button
                                  type="button"
                                  onClick={() => setIsConfirmingResetData(false)}
                                  className="bg-zinc-200 hover:bg-zinc-250 text-zinc-700 font-bold px-3 py-1.5 rounded-lg text-[10.5px] cursor-pointer"
                                >
                                  Cancel
                                </button>
                              </div>
                            ...
                            <button
                              type="button"
                              onClick={() => setIsConfirmingResetData(true)}
                              className="w-full bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold py-2 px-3 rounded-lg text-xs transition-colors border border-rose-200 cursor-pointer"
                            >
                        ...
                        <button
                          ...
                          className="w-full bg-zinc-200 hover:bg-zinc-250 text-zinc-700 font-bold py-2 px-4 rounded-xl text-xs transition-colors"
                        >
                          Sign Out of Session
                        </button>
                      </div>
